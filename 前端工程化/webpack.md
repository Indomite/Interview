...
![在这里插入图片描述](https://img-blog.csdnimg.cn/20201213234126274.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzQzNzU1NjQ2,size_16,color_FFFFFF,t_70)
...
通过`extract-text-webpack-plugin`插件，将CSS文件提取到一个单独的文件
...
1. 包括文件入口（单入口、多入口）
2. 输出（输出文件名，多个入口对应多个输出、hash值生成文件）
3. 资源发布（Url前缀、cdn）
4. 导出库
5. chunk文件
6. 异步加载
